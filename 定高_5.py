...
def proportional_guidance(fix_v=0.0, fix_h=0.0):
	...
	target_vel = np.array([-60, 20, -100])  # 目标初始速度(北-天-东) [m/s]

	# 在循环外初始化(定高弹道相关参数)
	missile_pos_1_max = 0
	integral_err_height = 0.0
	prev_err_height = 0.0
	Kp_height = 0.2
	Ki_height = 0.0001
	Kd_height = 0.9
	given_height = 1000
	height_control_yes = 0
	...
	for i in range(n_steps):
		...
		if np.linalg.norm(rel_pos) > 20e3:
			# 计算误差
			err_height = missile_pos[1] - given_height
			err_height = -err_height

			# 计算PID各项
			# 比例项
			P_height = Kp_height * err_height

			# 积分项（指数衰减近似）
			forgetting_factor = 1.0 - (1.0 / 1000)  # 近似等效于500个点的窗口
			integral_err_height = integral_err_height * forgetting_factor + err_height * dt
			I_height = Ki_height * integral_err_height

			# 微分项（使用后向差分）
			derivative_err_height = (err_height - prev_err_height) / dt
			D_height = Kd_height * derivative_err_height
			prev_err_height = err_height

			# 合成PID输出
			acc_cmd_v = P_height + I_height + D_height
		...
		# 记录符合高度的步数
		if abs(missile_pos[1] - given_height) <= 100:
			height_control_yes = height_control_yes + 1
		...
		err_h = acc_cmd_h + acc_b_z  # 这里的负号改为正号
	...
	print(f'missile_pos_1_max={missile_pos_1_max:.2f}')

	k_control_yes = height_control_yes / stop_i*100
	print(f'k_control_yes={k_control_yes:.2f}%')

	if plt_yes:
		plt.show(block=False)  # block=False 允许非阻塞显示
		plt.pause(1000)  # 暂停2秒
	# plt.savefig(f'./pid/{int(k_control_yes * 100)}_{KPi + 1}_{KIi + 1}_{KDi + 1}.png')
	plt.close()  # 关闭当前图像
...
# KP_list = [0.1, 0.01, 0.001, 0.0001, 0.00001, 0.000001]
# KI_list = [0.1, 0.01, 0.001, 0.0001, 0.00001, 0.000001]
# KD_list = [0.1, 0.01, 0.001, 0.0001, 0.00001, 0.000001]
# for KPi in range(len(KP_list)):
# 	for KIi in range(len(KI_list)):
# 		for KDi in range(len(KD_list)):
# 			KP = KP_list[KPi]
# 			KI = KP_list[KIi]
# 			KD = KP_list[KDi]
