...
迭代说明：
	2025-09-28:
		1) 基于“三维_比例_9.py”，使用“虚拟目标点法”，修改【详见版本1】
		2) 基于“三维_比例_9.py”，使用“虚拟指令加速度”，修改而来
		3) 采用err_loft的方法，减小误差
		4) 修改水平方向误差定义
迭代日期：2025-09-26
涉及到的坐标系：
	弹体坐标系(B系)：
...
# 生成虚拟目标点
# def target_virtual_point():
# 	return np.array([26000.0, -10000.0, 0.0])
...
# 三维比例导引
def proportional_guidance(fix_v=0.0, fix_h=0.0, fix_loft=0.0, plt_yes_in=False):
	plt_yes = False
	if fix_h != 0:
		plt_yes = True
	if plt_yes_in:
	...
	target_pos = np.array([46000.0, 0.0, 0.0])  # 目标初始位置(北-天-东) [m]
	target_pos_real = np.array([46000.0, 0.0, 0.0])  # 目标真实位置(北-天-东) [m]
	target_pos_virtual = np.array([26000.0, 10000.0, 0.0])  # 目标虚拟位置(北-天-东) [m]
	target_vel = np.array([0, 0, 0])  # 目标初始速度(北-天-东) [m/s]

	missile_pos_1_max = 0
	pass_loft_height = False
	given_loft_height = 5000

	# 仿真参数
	dt = 0.01  # 仿真时间步长 [s]
	t_total = 155.0  # 总仿真时间 [s]
	...
	target_pos_real_history = np.zeros((n_steps, 3))  # 存储目标真实位置(G系)
	...
	# 打印
	if plt_yes:
		if missile_I_zz_1 > missile_I_zz_t0:
	...
	for i in range(n_steps):
		...
		# 用给定指令加速度的方法(这里的数据需要提前计算，例如1548)
		loft_height = given_loft_height - fix_loft
		if missile_pos[1] < loft_height and not pass_loft_height:
			x = [0, loft_height]  # 导弹高度
			y = [20 * g, 0]  # 对应过载
			acc_cmd_v = np.interp(missile_pos[1], x, y)  # 线性插值
		else:
			acc_cmd_v = acc_cmd_v
			pass_loft_height = True

		# 记录导弹最大高度(便于分析超调量)
		if missile_pos[1] > missile_pos_1_max:
			missile_pos_1_max = missile_pos[1]
		...
		# 存储数据（顺序：舵面->姿态->速度->位置，T在前M在后）
		target_pos_real_history[i] = target_pos_real
	...
	print(f'loft_max={missile_pos_1_max:.2f}')

	err_loft = missile_pos_1_max - given_loft_height
...
if __name__ == "__main__":
	...
	for I in range(10):
		_, _, ERR_LOFT = proportional_guidance(fix_v=FIX_V, fix_h=FIX_H, fix_loft=FIX_LOFT, plt_yes_in=PLT_YES)
		FIX_LOFT = FIX_LOFT + ERR_LOFT * 0.9
		print(f'FIX_LOFT_{I + 1}={FIX_LOFT:.2f}')
		print(f'ERR_LOFT_{I + 1}={ERR_LOFT:.2f}')
		if abs(ERR_LOFT) < 100:
			break

	if abs(ERR_LOFT) > 100:
		print('LOFT IS NOT USEFUL!')
		exit()

	PLT_YES = True
	_, _, ERR_LOFT = proportional_guidance(fix_v=FIX_V, fix_h=FIX_H, fix_loft=FIX_LOFT, plt_yes_in=PLT_YES)
	FIX_LOFT = FIX_LOFT + ERR_LOFT * 0.9
	print(f'FIX_LOFT={FIX_LOFT:.2f}')
	print(f'ERR_LOFT={ERR_LOFT:.2f}')
